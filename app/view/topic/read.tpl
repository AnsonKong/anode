...
{% block content %}
	...
	<div class="container rounded-top bg-light mt-3 p-0">
		...
		<div id="topicHead" class="p-2 pt-4">
	  	...
	  	<small class="text-muted">作者 <a class="text-muted" href="/user/{{ topic.user.id }}">{{ topic.user.email }}</a></small>
		...
		<hr class="my-2">
		<div class="p-3">{{ helper.decodeBase64(topic.content) | safe }}</div>
	</div>
	<!-- 回复面板 -->
		...
		<li class="list-group-item border-left-0 border-right-0 border-top-1 border-bottom-0 rounded-0 p-2">
			<div class="row no-gutters">
				...
				<div class="col">
					<div class="d-flex">
						<span class="mr-auto">
							<small><a class="text-dark replyText" href="/user/{{ item.user.id }}">{{ item.user.email }}</a></small>
							<small><a href="#">{{ loop.index }}楼•{{ helper.fromNow(item.created_time) }}</a></small>
					...
					<div class="pl-2">
						{{ helper.decodeBase64(item.content) | safe }}
					</div>
				</div>
			</div>
		</li>
