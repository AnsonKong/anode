...
exports.parseMarkdown = content => {
  ...
  while (startIndex !== -1) {
    const endIndex = tail.indexOf('</a>');
    if (endIndex === -1) break;
    head += tail.slice(0, startIndex);
    const tempStr = tail.substr(startIndex, 2) + ' target="_blank" ' + tail.slice(startIndex + 2, endIndex + 4);
    // next round
    head += tempStr;
...
exports.addQuery = (ctx, key, value) => {
  let result = ctx.path;
  const cloneQuery = Object.create(ctx.query);
  cloneQuery[key] = value;
  let first = true;
  for (const i in cloneQuery) {
