...
function buildRepliesHistory(parentIds) {
  ...
  for (let i = 0; i < parentIds.length; i++) {
    const latestParentReplyContent = $('#' + parentIds[i] + ' .reply-content');
    const text = latestParentReplyContent.get(0).innerText;

    const latestParentAvatarContainer = $('#' + parentIds[i] + ' .reply-avatar-container');
    const aNode = latestParentAvatarContainer.get(0).cloneNode(true);
    $(aNode).attr('target', '_blank');
    $(aNode).css('text-decoration', 'none');

    const newLi = $(`<li class="list-group-item">${aNode.outerHTML}${filterXSS(text)}<a href="#${parentIds[i]}">↑</a></li>`);
